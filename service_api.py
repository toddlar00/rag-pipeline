#!/usr/bin/env python3
"""Authenticated, loopback-only HTTP adapter for the RAG application service."""
...
def create_app(
    ...
    @app.exception_handler(service_runtime.ServiceRuntimeError)
    ...
    async def list_jobs(request: Request):
        ...
        if limit_value is None:
            limit = service_runtime.DEFAULT_JOB_PAGE_LIMIT
        else:
            limit = _bounded_decimal(
                limit_value, minimum=1,
                maximum=service_runtime.MAX_JOB_PAGE_LIMIT)
        cursor = (
