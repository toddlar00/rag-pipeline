...
| Evaluation and runtime dependency boundaries | Implemented locally | R8a/R8b extract strict evaluation inputs and the query/judgment domain; R8c-1 removes the durable engine's facade dependency with one frozen supervision capability; R8c-2 removes the service host's facade dependency with a frozen host binding, shared embedding/lease policy, and a dedicated physical-search child; R8c-3 moves durable coordination inward, retains a compatible manager shell, and gives the service one frozen launch/reconcile/integrity generation. The tracked graph is acyclic and direct tests cover validation, atomic snapshots, import isolation, error/cleanup propagation, exact coordination/search wiring, and crash-released cross-process exclusion. R7 and R8 remain open for broader static gates and one application composition root; the local work is unpublished |
...
- `main` is `54cdb00`. The last published exact head is `e2196a1` on PR #43, 33
  ...
  local branch adds thirteen commits after that head: repository truth,
  ...
  runtime-supervision binding, the R8c-2 service-host inversion, and the R8c-3
  job-coordination inversion. None is yet a published
...
- The pre-R0A statement-coverage probe reported 75% over application and tool
  ...
  does not currently measure coverage. The R8c-3 tree passes 2,038 tests with 7
...
- CI is broad across Linux, Windows, Python 3.10-3.14, the full CPU environment,
  ...
  Git-tracked Python sources; the R8c-3 inventory contains 138 files.
...
| Local sequence from `88fd301` through R8c-3 | Validated local slices; no remote branch/PR | Complete exact-tree review, restore workflow-capable GitHub authentication, freeze the exact SHA, then publish only as one current candidate |
...
| R8 dependency direction | Evaluation inversion, durable supervision binding, service-host/search inversion, and service job-coordination inversion implemented locally | Exact-function extraction, facade/type compatibility, import order/isolation, atomic bindings, cleanup/failure propagation, exact job/search wiring, shared lock identity, and cross-process tests cover an acyclic first-party graph | Not published | No owner decision | Converge the service/API/UI/CLI composition roots without removing proven facades prematurely |
...
| Dissolve runtime dependency seams | Evaluation release/review use `evaluation_inputs.py`, evaluator/review use `evaluation_queries.py`, R8c-1 moves durable-job supervision into a frozen binding, R8c-2 moves service supervision/model/lease defaults into another frozen binding with a dedicated search child, and R8c-3 moves launch/recovery into `job_coordination.py` behind a stable manager facade and frozen service capability. The first-party graph is acyclic; `rag.py`, the service API, and UI remain split composition/CLI roots | Retain the enforced acyclic baseline; converge application roots one characterized boundary at a time |
...
loader/query staging, and the acyclic evaluation graph; cross-surface
  ...
  R8c-1/R8c-2/R8c-3 capabilities and isolation boundaries. Existing
  ...
  import graph is acyclic. The exact R8c-3 tree passes 2,038 tests with 7
  platform skips and 429 dependency warnings; all 138 tracked Python sources
  compile, and Ruff, dependency/model-artifact policy, and diff gates pass. R8
  remains open and unpublished for one application composition root.
...
| 7 | Continue application-root R8 dependency direction and begin R9 orchestration decomposition | Evaluation inversion, durable supervision, service-host/search, job coordination, compatibility facades, and the acyclic-DAG gate are local; root convergence still requires the rest of R7 and the Phase A baselines |
