...
comparison; the candidate published as draft
[#76](https://github.com/toddlar00/rag-pipeline/pull/76), passed all 27
hosted checks at its exact head `ddcbe89` — including both hosted Phase A0
cells with retained evidence bundles — and merged to `main` on 2026-08-01
with the owner merge decision as its review gate. PRs #66-#71 and #73 closed
...
independent complete same-platform 9×5 comparison; it passed all 27 hosted
checks at its exact #83 head and merged on 2026-08-01. On 2026-08-02 the
read-only first-pass PDF triage command (`rag.py scan`: deterministic
report card with preprocess forecast, OCR recommendation, watermark and
TOC detectability — spec and plan under `docs/superpowers/`) merged to
